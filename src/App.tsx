...
function App() {
  ...
  const onSendChanges = () => {
    const sortArr: ISpeedLimit[] = speedLimitList.sort((a, b) =>
      a.speedLimit < b.speedLimit ? -1 : 1
    );
    const desc: string = trains.filter((item) => item.name === selectTrain)[0]
      .description;
    ...
    <div className="container">
      ...
      <div className="tables-area">
        ...
        {selectTrain.length > 0 ? (
          ...
          <div className="select-train">Выберите поезд</div>
        )}
        <button
          className={drowBtn()}
          disabled={!change}
